...
CREATE INDEX IF NOT EXISTS idx_defects_item_id ON defects(item_id);
CREATE INDEX IF NOT EXISTS idx_defects_type_id ON defects(type_id);

CREATE INDEX IF NOT EXISTS idx_repairs_item_id ON repairs(item_id);
CREATE INDEX IF NOT EXISTS idx_repairs_master_id ON repairs(master_id);
CREATE INDEX IF NOT EXISTS idx_repairs_operation_id ON repairs(operation_id);
CREATE INDEX IF NOT EXISTS idx_repairs_completed_at ON repairs(completed_at);

CREATE INDEX IF NOT EXISTS idx_clients_phone ON clients(phone);
CREATE INDEX IF NOT EXISTS idx_clients_name_surname ON clients(surname, name);
...
('Дыра', 3),
('Порванная молния', 2),
('Отстёгнутая пуговица', 1),
('Повреждённая подкладка', 3),
('Разошедшийся шов', 2),
('Пятно (не химчистка)', 1),
('Изношенный воротник/манжеты', 4);

INSERT INTO repair_operations (name, default_base_price, default_complexity_factor) VALUES
('Зашить дыру', 300.00, 1.0),
('Заменить молнию', 600.00, 1.3),
('Пришить пуговицу', 100.00, 0.5),
('Восстановить подкладку', 800.00, 1.8),
('Перешить шов', 400.00, 1.1),
('Удалить/заменить этикетку', 150.00, 0.6),
('Укрепить воротник', 500.00, 1.4),
('Полный рестайлинг элемента', 2000.00, 2.5);
